...
} from '../../core/hooks/useMixpanelTracker';
...
describe('useMixpanelTracker', () => {
  ...
  describe('useMixpanelTrackerSession', () => {
    it('should implement useSessionStorage hook', () => {
      ...
      jest.mocked(useSessionStorage).mockReturnValue([previousPage, jest.fn(), jest.fn()]);

      const { result } = renderHook(() => useMixpanelTrackerSession());

      expect(result.current).toEqual([previousPage, expect.any(Function), expect.any(Function)]);
    });
  });
  ...
  describe('useSaveTrackerParamsOnNavigation', () => {
    it('should update session storage with previous page and scroll position on route change', () => {
      ...
      jest.mocked(useSessionStorage).mockReturnValue([{ previousPage: '', scroll: '' }, mockUpdateFn, jest.fn()]);
      ...
      // Check that the event listener is removed on cleanup
      unmount();
      expect(mockRouterEvents.off).toHaveBeenCalledWith('routeChangeStart', saveRouteInfoOnNavigation);
    });
  });
});
