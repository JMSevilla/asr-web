...
describe('Business navigation logic', () => {
  describe('openInNewTab', () => {
    it('should create link with attributes and open it', () => {
      ...
      expect(anchorMocked.setAttribute.mock.calls[1]).toEqual(['target', '_blank']);
      expect(anchorMocked.setAttribute.mock.calls[2]).toEqual(['rel', 'noopener,noreferrer']);
  ...
  describe('openInNewWindow', () => {
    it('should open window with correct arguments', () => {
      ...
      expect(spyWindowOpen.mock.calls[0][2]).toBe('noopener,noreferrer,resizable');
    });
  });
});
