...
import { useChatBotConversation, useCloseOnEscClick, useTrapContainerFocus } from '../../../components/chatBot/hooks';
...
describe('ChatBot hooks', () => {
  describe('useChatBotConversation', () => {
    ...
    it('starts conversation by executing API callback', () => {
      const mockApiCallback = { loading: false, execute: jest.fn(), result: { data: null } } as any;
      jest.mocked(useApiCallback).mockReturnValue(mockApiCallback);
      jest.mocked(useCachedAccessKey).mockReturnValue({ data: { contentAccessKey: '123' } } as any);
    ...
    it('selects an option by executing API callback with option key', () => {
      const mockApiCallback = { loading: false, execute: jest.fn(), result: { data: DEFAULT_OPTION } } as any;
      jest.mocked(useApiCallback).mockReturnValue(mockApiCallback);
      jest.mocked(useCachedAccessKey).mockReturnValue({ data: { contentAccessKey: '123' } } as any);
