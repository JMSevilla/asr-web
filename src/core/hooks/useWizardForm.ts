...
export function useWizardForm<TKey extends StepKey, TFormValue, TProps>(
  ...
  defaultStep: TKey,
  onChange?: (value: Partial<TFormValue>, filter?: boolean, isDirty?: boolean) => void,
  dependantsExist?: boolean,
) {
  const { scrollTop } = useScroll();
  const router = useRouter();
  const { hideNotifications } = useNotificationsContext();
  const [step, setStep] = useState<{ currentStep: TKey; previousStep?: TKey }>({ currentStep: defaultStep });
  const [values, setValues] = useState<Partial<TFormValue>>({} as Partial<TFormValue>);
    ...
    (newValues: TFormValue, filter?: boolean, isDirty?: boolean) => {
      ...
      const next =
        nextStepValue instanceof Function
          ? nextStepValue({ values: newValues, router, previousStep: step.currentStep, dependantsExist, stepHistory })
          : nextStepValue;
      ...
      hideNotifications();
      scrollTop();
      setStepHistory(prev => (next === 'SummaryView' ? [] : [...prev, step.currentStep]));
  ...
  const previousStep = useCallback(() => {
    ...
    const previous =
      previousStepValue instanceof Function
        ? previousStepValue({ values, router, previousStep: step.previousStep, dependantsExist, stepHistory })
        : previousStepValue;
...
type NextStep<TKey, TFormValue> = TKey | ((props: NextStepProps<TFormValue, TKey>) => void | TKey);
...
export type WizardFormMap<TKey extends StepKey, TFormValue, TProps> = {
  [k in TKey]: {
    ...
    content(stepProps: WizardFormProps<TFormValue, TKey> & TProps): ReactElement;
  };
};
