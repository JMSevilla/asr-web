...
export const useCookie = <T,>(
  key: string,
): [T | null, (value: T | null, options?: CookieSetOptions) => void, VoidFunction] => {
  const [cookie, updateCookie, removeCookie] = useReactCookies<string, CookieValue<T>>([key]);

  const set = useCallback(
    (value: T | null, options?: CookieSetOptions) => {
      updateCookie(key, value, options ?? { path: '/', sameSite: true, secure: process.env.NODE_ENV === 'production' });
    },
    [key],
  );
