...
export const useCachedCmsGlobals = (tenantUrl: string) => {
  const [globals, setGlobals] = useSessionStorage<GlobalsSessionStorage | null>('cms-globals', null);
  const cmsGlobalsCb = useApiCallback(async (api, contentAccessKey?: string) => {
    const result = await api.content.globals(tenantUrl, contentAccessKey);
  ...
  function handleGlobalsFetch(contentAccessKey?: string) {
    if (globals && tenantUrl === globals.tenantUrl && contentAccessKey === globals.contentAccessKey) {
