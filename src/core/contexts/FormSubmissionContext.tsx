...
type Callback = { key: string; enabled: boolean; unchanged: boolean; fn?: AsyncFunction };
...
function reducer(state: State, action: Action) {
  switch (action.type) {
    ...
    case 'init':
      return {
        ...state,
        callbacks: [...state.callbacks.filter(cb => cb.key !== action.payload.key), action.payload],
      };
    case 'toggleCallback': {
      return {
        ...state,
        callbacks: state.callbacks.map(cb =>
          cb.key === action.payload.key
            ? { ...cb, enabled: action.payload.enabled, unchanged: action.payload.unchanged }
            : cb,
        ),
      };
    }
...
const context = createContext<{
  ...
  toggleCallback: (payload: { enabled: boolean; unchanged: boolean; key: string }) => void;
}>({} as any);

export const FormSubmissionContextProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  const [state, dispatch] = useReducer(reducer, INITIAL_STATE);
  const [loading, setLoading] = useState(false);

  const submitFn = async () => {
    if (!state.callbacks) {
      throw new Error('No form bound to form-submission-action found in this page');
    }
    setLoading(true);
    try {
    ...
    <context.Provider
      value={useMemo(
        () => ({
          ...
          toggleCallback: (payload: TogglePayload) => dispatch({ type: 'toggleCallback', payload }),
