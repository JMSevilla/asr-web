...
export const shouldReduceSpaceBetweenCmsBlocks = (index: number, blocks?: PageContentValues[] | null) => {
  if (!blocks?.length) return false;
  return (
    areCurrentAndNextBlocksAreMessages(index, blocks) ||
    areCurrentAndNextBlocksAreAccordions(index, blocks) ||
    isBlockABackButton(blocks?.[index])
  );
};

const areCurrentAndNextBlocksAreMessages = (index: number, blocks?: PageContentValues[] | null) =>
  blocks?.[index]?.type === 'Message' && blocks?.[index + 1]?.type === 'Message';
const areCurrentAndNextBlocksAreAccordions = (index: number, blocks?: PageContentValues[] | null) =>
