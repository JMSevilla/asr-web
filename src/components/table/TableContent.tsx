...
import { Box, Grid, Table, TableBody, TableCell, TableHead, TableRow, Typography, useTheme } from '@mui/material';
            ...
            tableInstance.headerGroups.map(headerGroup => (
              <TableRow {...headerGroup.getHeaderGroupProps()} key={headerGroup.getHeaderGroupProps().key}>
                {headerGroup.headers
                  ...
                  .map((column, index) => {
                    const filterLabel = filterLabels?.find(filterLabel => filterLabel.id === column.id);
                    ...
                    return (
                      <TableCell
                        ...
                        colSpan={!index ? 2 : 1}
                        style={{ border: 0, paddingBottom: 0, paddingLeft: index === 0 ? 0 : 16 }}
                      >
          ...
          {filters &&
            tableInstance.headerGroups.map(headerGroup => (
              <TableRow
                ...
                key={headerGroup.getHeaderGroupProps().key.toString() + filtersKey}
              >
                {headerGroup.headers
                  .filter(column => column.id !== '_selector')
                  .map((column, index) => (
                    <TableCell
                      ...
                      style={{
                        ...
                        paddingRight: index === headerGroup.headers.length - 2 ? 0 : 16,
                      }}
                    >
                ...
                >
                  {row.cells.map(cell => (
                    <TableCell
                      ...
                      onClick={() => cell.column.id !== '_selector' && onRowClicked?.(cell.row)}
                      style={{
                    ...
                    >
                      <Typography color="inherit" variant="body2" fontWeight={row.original?.isBold ? 'bold' : 'normal'}>
                        {isValidDate(new Date(cell.value)) ? formatDate(cell.value) : cell.render('Cell')}
                      </Typography>
                    </TableCell>
                  ))}
                </TableRow>
        ...
        <Box width="100%" mt={12} display="flex" justifyContent="flex-end">
          ...
          >
            <Box display="flex" mr={2}>
              <DownloadIcon customColor={downloadDisabled ? theme.palette.grey.A400 : theme.palette.common.white} />
            </Box>
            {downloadLabel}
          </PrimaryButton>
        </Box>
