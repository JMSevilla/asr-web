...
import { PaginationData } from '../../api/mdp/types';
...
import { TableContentMobile, TableContentMobileFiltersConfig } from './TableContentMobile';
...
export interface TableProperties<T extends Record<string, unknown>> extends TableOptions<T> {
  ...
  additionalFilterValues?: { label: string; value: boolean | number | string }[];
...
const DefaultHeader: React.FC<HeaderProps<{}>> = ({ column }) => <>{column.id.startsWith('_') ? null : column.id}</>;
...
export function PaginatedTable<T extends Record<string, unknown>>(
  ...
  const [additionalFilter, setAdditionalFilter] = useState<{ id: string; value: string | boolean | number }[]>([]);
  ...
  const dateBetweenFilterFn = useMemo(
    () => (rows: Row<T>[], id: IdType<T>[], filterValues: FilterValue) => {
      ...
      return rows.filter(row => {
        const time = parse(row.values[id[0]], 'yyyy-MM-dd HH:mm:ss.SSS', new Date());
        return filterValues.length === 0 ? rows : time >= startDate && time <= endDate;
      });
    },
    [],
  );

  const filterTypes = useMemo(() => ({ dateBetween: dateBetweenFilterFn }), [dateBetweenFilterFn]);
    ...
    (hooks: Hooks<T>) => {
      hooks.visibleColumns.push(columns => [
            ...
            <Grid container alignItems="center" wrap="nowrap">
              <Grid item>
                <CheckboxCell
                  ...
                  title={`${
                    getToggleAllPageRowsSelectedProps().checked ? 'Unselect' : 'Select'
              ...
              {!!additionalFilterValues?.length && (
                <Grid item>
                  <IconButton
                    ...
                    title={labelByKey(`${labelPrefix}filter_list`)}
                  >
                    <KeyboardArrowDown fontSize="medium" sx={{ color: 'common.black' }} />
                  </IconButton>
                </Grid>
              )}
            </Grid>
          ...
          Cell: ({ row }: CellProps<T>) => {
            return (
              <CheckboxCell
                ...
                title={`${row.getToggleRowSelectedProps().checked ? 'Unselect' : 'Select'} retirement application`}
              />
            );
          },
          minWidth: 73,
        ...
        ...columns,
        {
          ...
          Cell: ({ row }: CellProps<T>) => <div>{row.original.actions as ReactNode}</div>,
          minWidth: 300,
          width: 400,
          maxWidth: 450,
        },
  ...
  function handleAdditionalFilterSelect(value?: string) {
    ...
    tableInstance.setAllFilters([...filters, { id: additionalFilterColumn, value }]);
    onFilterChange && onFilterChange([...filters, { id: additionalFilterColumn, value }]);
  }
