  ...
  return (
    <>
      ...
      >
        <Box width={650} p={4}>
          <CustomCalendar
            ...
            nextAriaLabel={labelByKey('next_month_aria_label')}
            next2AriaLabel={labelByKey('next_year_aria_label')}
            prevLabel={<KeyboardArrowLeft />}
            prev2Label={<KeyboardDoubleArrowLeft />}
            prevAriaLabel={labelByKey('prev_month_aria_label')}
            prev2AriaLabel={labelByKey('prev_year_aria_label')}
          />
        </Box>
      </Popover>
      ...
      <TextField
        ...
        onClick={(e: React.MouseEvent<HTMLInputElement>) => handleInputClick(e.currentTarget)}
        onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) =>
          e.code === 'Enter' && handleInputClick(e.currentTarget)
        }
        inputProps={{ placeholder: labelByKey(`${labelPrefix}select_a_date_range`) }}
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <CalendarToday sx={{ color: theme => theme.palette.common.black }} />
            </InputAdornment>
          ),
        }}
      />
...
const formatValue = (value: DateRange) =>
  value[0] || value[1] ? `${value[0] ? formatDate(value[0]) : ''} - ${value[1] ? formatDate(value[1]) : ''}` : '';

const CustomCalendar = styled((props: CalendarProps) => <Calendar {...props} />)(({ theme }) => ({
