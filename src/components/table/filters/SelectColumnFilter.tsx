...
export const SelectColumnFilter: React.FC<Props> = ({
  ...
  return (
    <Select
      ...
      sx={{
        color: theme => (value ? 'common.black' : theme.palette.appColors.essential[300]),
      }}
      renderValue={value => value || labelByKey(`${labelPrefix}select_${id}`)}
      IconComponent={() => <Tune sx={{ color: theme => theme.palette.common.black, margin: 3 }} />}
      MenuProps={{
        anchorOrigin: {
          vertical: 'bottom',
          horizontal: 'left',
    ...
    >
      ...
      >
        <Box py={2}>{labelByKey(`${labelPrefix}all`)}</Box>
      </MenuItem>
