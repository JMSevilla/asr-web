...
import { ClickAwayListener, Divider, Grid, Grow, Paper, Popper, PopperProps, Theme, Typography } from '@mui/material';
...
interface Props {
  ...
  onClosed(): void;
  onFilterSelected(value?: boolean | number | string | null, desc?: boolean): void;
}
              ...
              >
                <Grid item xs={12} sx={infoRowStyle}>
                  <Grid
                    ...
                    bgcolor={isNoneSelected() ? 'appColors.support80.transparentLight' : 'transparent'}
                  >
                    <Grid item>
                      <Typography variant="body2" ml={8} color={isNoneSelected() ? 'primary' : 'inherit'}>
                        {labelByKey(`${labelPrefix}all`)}
                      </Typography>
                    </Grid>
                    <Grid item pt={2}>
                      <KeyboardArrowDown
                        fontSize="medium"
                        sx={{ color: isNoneSelected() ? 'appColors.primary' : 'common.black' }}
                      />
                    </Grid>
                  </Grid>
                </Grid>
                {values.map((value, index) => (
                  <Grid key={index} container xs={12} alignItems="center">
                    ...
                    <Grid
                      ...
                      onKeyDown={e => e.code === 'Enter' && onFilterSelected(value.value)}
                      bgcolor={isSelected(value) ? 'appColors.support80.transparentLight' : 'transparent'}
                    >
                      <Grid container alignItems="center">
                        <Grid item>
                          <Typography variant="body2" ml={8} color={isSelected(value) ? 'primary' : 'inherit'}>
                            {labelByKey(`${labelPrefix}${value.label}`)}
                          </Typography>
                        </Grid>
                        <Grid item pt={2}>
                          <KeyboardArrowDown
                            fontSize="medium"
                            sx={{ color: isSelected(value) ? 'appColors.primary' : 'common.black' }}
                          />
                        </Grid>
                      </Grid>
                    </Grid>
