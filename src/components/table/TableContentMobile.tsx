...
import { ErrorBox, ListLoader, PrimaryButton } from '..';
...
export type TableContentMobileFiltersConfig<T> = { alwaysOnFilters: (keyof T)[]; menuFilters?: (keyof T)[] };
...
export const TableContentMobile = <T extends Record<string, unknown>>({
  ...
  const canDownload = !downloadHidden && !downloadDisabled && !!onDownloadClicked;
  const canClearFilters = !!filters.length;

  useEffect(() => {
    tableInstance.headerGroups.forEach(hg =>
      hg.headers.forEach(col => {
        if (Array.isArray(filters)) col.filterValue = filters.find(f => f.id === col.id)?.value;
      }),
    );
    ...
    <>
      <Table {...tableInstance.getTableProps()}>
        <Grid container spacing={4} mb={4} key={filtersKey}>
          {renderFilters()}
          <Grid item xs={12}>
            <PrimaryButton onClick={onSearchClicked} disabled={!filters.length} fullWidth data-testid="search-button">
              {labelByKey(`${labelPrefix}search`)}
            </PrimaryButton>
          </Grid>
        </Grid>
        <Box
        ...
        >
          <Box flex={1} minHeight={35} display="flex" justifyContent="space-between" alignItems="center" gap={2}>
            {canDownload && (
              <PrimaryButton
            ...
            {canDownload && (
              ...
              >
                {labelByKey(`${labelPrefix}clear`)}
              </TextButton>
            )}
            {canClearFilters && (
              <TextButton onClick={handleFiltersClear} sx={actionButtonStyle} data-testid="clear-filters">
            ...
            {!canDownload && !canClearFilters && (
              <Link
                ...
                gap={1}
              >
                {labelByKey(`${labelPrefix}filter_results`)} <TuneIcon />
              </Link>
            )}
          </Box>
        ...
        <TableBody {...tableInstance.getTableBodyProps()}>
          ...
          {!tableInstance.page.length && !isLoading && (
            <TableRow data-testid="table-data-not-found-row">
              <TableCell colSpan={tableInstance.visibleColumns.length}>
                <ErrorBox mb={0} label={filters.length ? noDataFoundText : noDataText} />
              </TableCell>
            </TableRow>
          )}
          {!isLoading &&
              ...
              return (
                ...
                >
                  <TableCell
                    style={{
                      display: 'flex',
                      color: row.isSelected ? theme.palette.primary.main : undefined,
                      borderColor: theme.palette.grey.A400,
                    }}
                  >
                    <CheckboxCell {...row.getToggleRowSelectedProps()} sx={{ mr: 3 }} />
                    <Box flex={1} onClick={() => onRowClicked?.(row)} sx={{ cursor: 'pointer' }}>
                      {renderRow(row)}
                    </Box>
                  </TableCell>
                </TableRow>
        ...
        .map(column => (
          <Grid item xs={12} key={column.getHeaderProps().key}>
            <Typography color="inherit" variant="body1">
              {filterLabels?.find(filterLabel => filterLabel.id === column.id)?.label}
            </Typography>
