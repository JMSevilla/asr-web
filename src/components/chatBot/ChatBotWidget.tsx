...
import { useChatBotConversation, useCloseOnEscClick, useTrapContainerFocus } from './hooks';
interface Props {}

export const ChatBotWidget: React.FC<Props> = ({}) => {
  const { startTalk, checkIntentAndStartGenesys, shouldHideDefaultChat, isGenesysEnabled, shouldHideHelpWidget, shouldHideLiveChat } = useGenesys();
  ...
  if (!isAuthenticated || shouldHideDefaultChat) {
    return null;
  }
  ...
  return (
    <ClickAwayListener onClickAway={handleClickAway}>
      ...
      >
        <ChatBotToggleButton className="fixed" isOpen={open} onOpen={handleOpen} onClose={handleClose} />
        <Paper
          className="fixed"
          role="region"
        ...
        >
          <Stack id="chatbot-window" width={360} ref={containerRef}>
            ...
            <ChatBotActions
              ...
              shouldHideHelpWidget={shouldHideHelpWidget}
              shouldHideLiveChat={shouldHideLiveChat}
            />
          </Stack>
        </Paper>
      </Stack>
  ...
  async function handleOpen() {
    const intentFound = await checkIntentAndStartGenesys();
    if (intentFound) {
      return;
    }
    setOpen(true);
    mixpanelTrackButtonClick({
      Category: 'Chatbot conversation start',
    });
    if (!chatBotConversation.loading && !chatBotConversation.allOptions.length) {
  ...
  function focusSkipContainer() {
    const firstFocusableSkipContainerElement = document
      ...
      ?.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])') as HTMLElement;
