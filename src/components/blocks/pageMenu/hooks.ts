...
export const useUpdateBlockInViewOnScroll = (items: PageMenuItem[]): [number, number, (value: number) => void] => {
  const scroll = useScroll();
  const allowAutoBlockIndexUpdate = useRef(true);
  const oldScrollPos = useRef(typeof window !== 'undefined' ? window.scrollY : 0);
  ...
  useEffect(() => {
    const updateBlockInView = () => {
      ...
      const bottomReached = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight;
      if (bottomReached) {
        return setIndex(items.length - 1);
      }

      const itemInViewport = items.findIndex(item => isElementInViewport(item.key.value));
      if (itemInViewport > -1) {
        const direction = oldScrollPos.current > window.scrollY ? 'up' : 'down';
        oldScrollPos.current = window.scrollY;
        return setIndex(oldIndex => (direction === 'down' ? Math.max(oldIndex, itemInViewport) : itemInViewport));
      }

      const everyItemIsOutsideViewport = items.every(item => isElementOutsideViewport(item.key.value));
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('scroll', allowAutomaticBlockInViewUpdates, false);
...
const useGradualIndex = (defaultIndex: number): [number, number, React.Dispatch<React.SetStateAction<number>>] => {
  ...
  useEffect(() => {
    let timeout: NodeJS.Timeout;
    if (index !== finalIndex) {
      timeout = setTimeout(() => setIndex(index > finalIndex ? index - 1 : index + 1), 100);
    }
...
function isElementOutsideViewport(id: string) {
  ...
  return rect ? rect.bottom < 0 || rect.top > (window.innerHeight || document.documentElement.clientHeight) : true;
}
