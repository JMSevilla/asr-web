...
import { TextWithBoldedFirstWord } from '../..';
import { parseDate } from '../../../business/dates';
...
export const DateField = <T extends FieldValues>({ name, control, defaultValue, isLoading, ...props }: Props<T>) => (
  <Controller<T>
    name={name}
    control={control}
    defaultValue={defaultValue}
    render={({ formState: _, ...controllerProps }) => <DateFieldComponent {...controllerProps} {...props} />}
  />
);
...
export const DateFieldComponent = <T extends object>({
  ...
  const value =
    typeof field?.value === 'string' ? startOfDay(parseDate(field?.value, defaultDatePickerFormat)!) : field?.value;
  return (
      ...
      <Grid item>
        <DatePicker
          ...
          onChange={val => {
            ...
            field?.onChange?.(returnRawValue ? [day, month, year].join('-') : val);
          }}
          value={value as Date | undefined}
          data-testid={props['data-testid'] || `${field?.name}-field`}
          inputProps={{ error: !!fieldState?.error?.message, 'data-testid': `${field?.name}-field` }}
  ...
  function renderLabel() {
    if (label) {
      return boldLabelFirstWord && typeof label === 'string' ? <TextWithBoldedFirstWord text={label} /> : label;
    }
