...
interface Props<T extends object> {
  ...
  withBorder?: boolean;
  valueParser?: (value: string) => UnpackNestedValue<FieldPathValue<T, FieldPath<T>>>;
}

type ComponentProps<T extends object> = Pick<RadioProps, 'checked' | 'disabled' | 'color' | 'inputProps'> &
...
const RadioButtonsComponent = forwardRef(function Component<T extends object>(
  { buttons, onChange, value, withBorder = false, valueParser, ...props }: Omit<ComponentProps<T>, 'ref'>,
  ref: ForwardedRef<HTMLButtonElement>,
) {
  return (
    <RadioGroup
      onChange={(_, value) => onChange(valueParser ? valueParser(value) : value)}
      ref={ref}
      value={value ?? ''}
      {...props}
    >
      <Grid container spacing={8}>
        {buttons.map(({ inputField, ...button }, index) => (
          <Fragment key={`${button.label}_${index}`}>
            <Grid item xs={12}>
              <QuestionRadioButton withBorder={withBorder} checked={button.value === value} {...button} />
            </Grid>
            {inputField && (
              <Grid
                item
                ...
                sx={{
                  ...
                  ':before': {
                    ...
                    backgroundColor: theme => theme.palette.appColors.secondary.transparentLight,
                    position: 'absolute',
                    top: '32px',
                    left: 0,
                  },
