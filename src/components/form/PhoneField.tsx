...
import { Grid, MenuItem, OutlinedInputProps, Select, Typography } from '@mui/material';
...
export const PhoneField = <T extends FieldValues>({ name, control, ...props }: Props<T>) => (
  <Controller<T>
    name={name}
    control={control}
    render={({ formState: _, ...controllerProps }) => <PhoneFieldComponent {...controllerProps} {...props} />}
  />
);
...
const PhoneFieldComponent = <T extends object>({
  ...
  const keys = fieldState?.error?.types ? Object.keys(fieldState.error.types) : [];
  const showErrorTooltip =
    isFocused && (keys?.includes('required') && keys.includes('min') ? keys.length > 2 : keys.length > 1);
  const phoneCodeList = useApi(api => (isAuthenticated ? api.mdp.phoneCodeList() : api.mdp.bereavementPhoneCodeList()));

  const phoneCodes = useMemo(() => {
    const data = phoneCodeList.result?.data ?? [];
    const defaultCode = data.find(pc => pc.code === DEFAULT_PHONE_COUNTRY_CODE);
