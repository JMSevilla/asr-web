...
export const TextAreaField = <T extends FieldValues>({ name, control, ...props }: Props<T>) => (
  <Controller<T>
    name={name}
    control={control}
    render={({ formState: _, field, ...controllerProps }) => {
      const { ref, ...nonRefField } = field;
      return <TextareaComponent {...controllerProps} {...props} {...nonRefField} />;
    }}
  />
);

interface ComponentProps<T extends object> extends Omit<Props<T>, 'name' | 'control'> {
  field?: ControllerRenderProps<T, Path<T>>;
  fieldState?: ControllerFieldState;
}

export const TextareaComponent = <T extends object>({ field, fieldState, ...props }: ComponentProps<T>) => {
      ...
      <Grid item>
        <TextareaAutosize
          ...
          style={{
            ...
            color: theme.palette.appColors.essential['300'],
            borderColor: theme.palette.appColors.incidental['075'],
            borderRadius: 0,
            resize: 'none',
          }}
