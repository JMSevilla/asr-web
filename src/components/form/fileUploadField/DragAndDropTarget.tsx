...
export const DragAndDropTarget: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  return (
    <Stack
      ...
      border={theme =>
        `dashed 2px ${
          isLoading ? theme.palette.appColors.tertiary.transparentDark : theme.palette.appColors.tertiary.light
        }`
      }
      borderRadius="4px"
      bgcolor={isLoading ? 'appColors.support80.transparentDark' : 'appColors.support80.transparentLight'}
      alignItems="center"
      justifyContent="flex-start"
      gap={4}
      color={theme => theme.palette.appColors.essential[500]}
    >
      {isActive && !isLoading ? (
        dragActiveLabel ? (
