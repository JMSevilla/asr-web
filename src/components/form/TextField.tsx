...
export const TextField = <T extends FieldValues>({ name, control, defaultValue, ...props }: Props<T>) => (
  <Controller<T>
    name={name}
    control={control}
    defaultValue={defaultValue}
    render={({ formState: _, ...controllerProps }) => <TextFieldComponent {...controllerProps} {...props} />}
  />
);
...
export const TextFieldComponent = <T extends object>({
  ...
  const showErrorTooltip =
    !errorTooltipDisabled && isFocused && fieldState?.error?.types && Object.keys(fieldState.error.types).length > 1;

  return (
    <Grid container spacing={2} direction="column">
      <Grid item>{fieldState?.error?.message ? showErrorBelowLabelFn() : label !== null && renderLabelFn()}</Grid>
      <Grid item>
